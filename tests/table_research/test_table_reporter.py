...
from my_agents.table_research.table_reporter import TableReporter


def _setup_mdformat(monkeypatch, fn):
    """Replace mdformat.text with given function."""
    module = __import__(
        "my_agents.table_research.table_reporter",
        fromlist=["mdformat"],
    )
...
def test_reporter_fallback_template(monkeypatch):
    # Simulate Jinja2 not installed
    import my_agents.table_research.table_reporter as reporter_mod

    monkeypatch.setattr(reporter_mod, "Environment", None)
    reporter = reporter_mod.TableReporter()
    state = {
        "table_name": "demo.Table",
        "report_parts": {"key_columns": ["id"], "sample_queries": ["select 1"]},
    }
